...
const ManageScholarships = () => {
  ...
  mutationFn: (updatedData) =>
    axiosSecure.patch(`/scholarships/${selectedScholarship._id}`, updatedData),
  onSuccess: () => {
              ...
              {scholarships.map((sch) => (
                <tr key={sch._id}>
                  ...
                  <td className="flex gap-2">
                    <button className="btn btn-sm" onClick={() => handleDetails(sch)}>
                      <FaInfoCircle />
                    </button>
                    <button className="btn btn-sm btn-warning" onClick={() => handleEdit(sch)}>
                      <FaEdit />
                    </button>
                    <button className="btn btn-sm btn-error" onClick={() => handleDelete(sch._id)}>
                      <FaTrashAlt />
                    </button>
                  </td>
                </tr>
        ...
        <div className="fixed inset-0 z-50 bg-black bg-opacity-30 flex items-center justify-center">
          <div className="bg-white p-6 rounded shadow-lg max-w-xl w-full">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <input {...register("scholarshipName")} placeholder="Scholarship Name" className="input input-bordered w-full" />
              <input {...register("universityName")} placeholder="University Name" className="input input-bordered w-full" />
              <input {...register("subjectCategory")} placeholder="Subject Category" className="input input-bordered w-full" />
              <input {...register("degree")} placeholder="Degree" className="input input-bordered w-full" />
              <input {...register("applicationFees")} type="number" placeholder="Application Fees" className="input input-bordered w-full" />

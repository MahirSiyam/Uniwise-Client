  ...
  return (
    <div className="my-12">
      ...
      <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
        {scholarships.map((item) => (
          <div key={item._id} className="card bg-base-300 shadow-xl">
            {/* <figure> */}
              <img src={item.universityImage} alt={item.universityName} className="h-[400px] w-[550px] object-cover rounded-t-lg" />
            {/* </figure> */}
            <div className="card-body">
              ...
              <p className="text-sm"><strong>Posted:</strong> {new Date(item.postDate).toDateString()}</p>
              <div className="card-actions justify-end">
                <Link to={`/scholarship/${item._id}`} className="btn bg-red-600 hover:bg-red-700 btn-sm text-white">
                  Details
                </Link>
              </div>
            </div>
      ...
      <div className="text-center mt-12"> 
        <Link to="/allScholarship" className="btn bg-red-600 hover:bg-red-700 btn-md text-white">
